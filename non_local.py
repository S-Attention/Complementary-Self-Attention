...
def sn_non_local_block_sim(x, update_collection, name, init=tf.contrib.layers.xavier_initializer()):
  with tf.variable_scope(name):
    ...
    attn_01 = tf.matmul(theta_01, phi_01, transpose_b=True)
    ...
    attn_11 = tf.matmul(theta_11, phi_11, transpose_b=True)
    ...
    attn_dct=attn_00+attn_01+attn_10+attn_11
